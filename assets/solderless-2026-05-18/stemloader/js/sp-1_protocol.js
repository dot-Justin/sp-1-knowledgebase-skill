/**
 * basic structure for converting and transferring audio to the SP-1
 */
...
// Album metadata read errors
const ALBUM_ERR = {
  OK: 0,
  MAGIC_NOT_FOUND: 1, // 'ALBUM_PRESENT' not found
  ALBUM_TOO_SHORT: 2, // album_len < 2
  ALBUM_TOO_LONG: 3, // album_len > 0x7FFFF (524287)
  TOO_MANY_SONGS: 4, // num_songs > 60
  ALBUM_TITLE_TOO_LONG: 5, // album_title_len > 63
  ALBUM_TITLE_ALLOC_FAILED: 6, // album title alloc failed
  SONG_INDEX_ALLOC_FAILED: 7, // song index alloc failed
  SONG_OFFSET_INVALID: 8, // song offset out of order / invalid
  SONG_LENGTH_TOO_LARGE: 9, // song_len > 999, 999
  SONG_EXCEEDS_ALBUM: 10, // song extends beyond album boundary
  ARTIST_NAME_TOO_LONG: 11, // artist_name > 62 bytes
  ARTIST_NAME_ALLOC_FAILED: 12, // artist_name alloc failed
  SONG_TITLE_TOO_LONG: 13, // song_title_len > 62 bytes
  SONG_TITLE_ALLOC_FAILED: 14, // Song title alloc failed
  MAGIC_NOT_FOUND_AT_END: 15, //  `ALBUM_PRESENT` not found at end of album
};

const OFFSET_MAGIC = 0;
const OFFSET_ALBUM_LEN = 13; // 4 bytes
const OFFSET_NUM_SONGS = 17; // 1 byte
const OFFSET_ALBUM_TITLE = 18; // 64 bytes - Album Title, null terminated and padded with X's
const OFFSET_SONGS = 82; //  List of song metadata
const STRING_LENGTH = 63; // bytes a string can take
const SONG_INDEX_LENGTH = 136; // number of bytes for a song entry
