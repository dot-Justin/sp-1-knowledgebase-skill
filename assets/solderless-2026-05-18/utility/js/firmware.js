...
$('btnConnectFlash').addEventListener('click', async () => {
  ...
  if (ok) {
    $('doneText').innerHTML = 'update complete.<br>Your device will restart now.';
    setState('fw-done');
  } else {
    setState('fw-error');
  }
...
async function flashFirmware(fw) {
  ...
  if (r0.payload.length >= 4) {
    logMsg(`  pages: ${readLE32(r0.payload, 0)}`);
    let payloadStr = r0.payload.join("");
    if (payloadStr == "10510") { // device in transfer mode
      deviceEnumerated = false;
      await sendCmd(0x51); // reset to bootloader
    }
  }

  while (!deviceEnumerated) { // wait to see device enumerated again
    $('flashText').textContent = 'waiting for device to enumerate...';
    await sleep(100);
  }
  ...
  // now auto reset?
  await sendCmd(0x50, new Uint8Array(), seq++, 3000);
  return true;
}


// serial event listener to pick up device enumerating
var deviceEnumerated = true;
navigator.serial.addEventListener("connect", deviceConnectEvent);
function deviceConnectEvent() {
